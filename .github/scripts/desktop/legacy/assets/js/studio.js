...
document.title = 'Studio · KFe Icons';
...
function loadIntoStudio(icon) {
  ...
  history.replaceState(null, '', `${window.location.pathname}?icon=${encodeURIComponent(icon.category + '/' + icon.name)}`);
  renderIconList();
  renderTree();
  renderProps();
...
saveConfirm.addEventListener('click', async () => {
  ...
  try {
    if (!window.kfe?.saveIcon) throw new Error('window.kfe bridge missing');
    const savedPath = await window.kfe.saveIcon({ category: cat, name, content: body });
    const data = { path: savedPath };
    ...
    if (state.selectedIcon?.name === name && state.selectedIcon?.category === cat) {
      ...
      history.replaceState(null, '', `${window.location.pathname}?icon=${encodeURIComponent(cat + '/' + name)}`);
    }
